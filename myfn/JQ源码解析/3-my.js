...
;(function (window,undefined) {
    ...
    myQuery.prototype.extend({
        ...
        attr:function (attr, value) {
            console.log(1);
        //判断是不是字符串
        if(myQuery.isString(attr)){
            // 一个参数,那么返回第一个元素的属性节点值
            if(arguments.length ==1){
                return this[0].getAttribute(attr);
            }
            // 如果传入两个参数,那么久遍历当前jq实例,取出每一个元素设置属性节点的值,
        ...
        //判断是否是对象
        else if(myQuery.isObject){
            //遍历传入的对象
            for(var key in attr){
                this.each(function () {
                    // 给每个遍历到的元素设置当前遍历的属性节点
        ...
        prop:function (attr, value) {
            //1 字符串
            if(myQuery.isString(attr)){
                //1.1 一个参数 返回第一个元素的属性节点
                if(arguments.length == 1){
                    return this[0][attr];
                }
                //1.2 两个参数,那么就遍历当前jq实例,取出每个元素进行设置属性节点的值
                else if(arguments.length == 2){
                    this.each(function () {
                        this[attr] = value;
                    })
